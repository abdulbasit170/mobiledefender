...
class MDN_ProductReturn_Helper_PendingProducts extends Mage_Core_Helper_Abstract
{
    ...
    public function getList()
    {
        ...
        $collection = mage::getModel('ProductReturn/RmaProducts')
            ...
            ->join('product_entity', 'entity_id = rp_product_id', 'sku');//Add sku -- 17015

        //Product price -- 17015
        $productPriceAttributeId = mage::getModel('eav/entity_attribute')->loadByCode('catalog_product', 'price')->getId();
        $collection->join('ProductReturn/CatalogProductEntityDecimal',
                '`product_entity`.entity_id=`ProductReturn/CatalogProductEntityDecimal`.entity_id '
                . 'and `ProductReturn/CatalogProductEntityDecimal`.store_id = 0 '
                . 'and `ProductReturn/CatalogProductEntityDecimal`.attribute_id = ' . $productPriceAttributeId)
        ->addExpressionFieldToSelect('price', 'ProductReturn/CatalogProductEntityDecimal.value', $productPriceAttributeId);
